...
from val_test_src_1 import *
from val_test_src_2 import *
...
def parse_args():
    ...
    # init
    parser.add_argument('-r', '--root', default='./', type=str)# or enter the current address, for example: /xxx/xxx/xxx/LPI-SKMSC
    parser.add_argument('-f', '--filename', default='RPI488', type=str)#RPI1847,RPI7317,RPI488

    # training config
    parser.add_argument('-e1', '--epochs', default=20, type=int)
    parser.add_argument('--AE_pretrain_epochs', default=15, type=int)
...
def main(args):
    ...
    DATA_SET = args.filename
    DATA_BASE_PATH = args.root +'/data/'  
    model_save_path_ = args.root + f"/result/{DATA_SET}/{DATA_SET}_result/models/"
    result_save_path = args.root
    # hyper params
    ...
    out = open(result_save_path + f'{DATA_SET}_test_result.txt', 'w')
    out.write('model_val_test\n')
    ...
    p_kmer, p_kmer_1, p_kmer_2, p_kmer_3, p_kmer_4,p_kmer_5, r_kmer, r_kmer_1, r_kmer_2, r_kmer_3, r_kmer_4,r_kmer_5, y = pre_process_data(samples=samples,
                            VECTOR_REPETITION_CNN=VECTOR_REPETITION_CNN,WINDOW_P_UPLIMIT=WINDOW_P_UPLIMIT,WINDOW_R_UPLIMIT=WINDOW_R_UPLIMIT)
    ...
    for fold in range(args.k_fold):
        # model save path
        model_save_path = model_save_path_ + 'fold_{}/'.format(fold)
        ...
        val_metrics_a_fold, test_metrics_a_fold = train_func(valloader, testloader,
                                                    args.epochs, fold, model_save_path,
